...
import { type Accessor, type AccessorDescriptor, Maybe } from './modifiers.ts';
...
export class Query<A extends Accessor | Accessor[], F extends Filter = Filter> {
  static intoArgument(
    ...
    const components = accessors.map((x: any) =>
      Maybe.isMaybe(x) ? x.type : x,
    );

    const initial = world.getArchetype(
      ...accessors.filter((x: any): x is Class => !Maybe.isMaybe(x)),
    );
  ...
  constructor(
    ...
    this.world.addEventListener('createTable', (table: any) => {
    ...
    for (
      ...
      for (let iterations = 0; iterations < length; iterations++) {
        for (let offset = 0; offset < componentCount; offset++) {
          elements[offset] = this.columns[columnGroup + offset + 1][iterations];
        }
  ...
  *pairs(): IterableIterator<[A, A]> {
   ...
   */
  private testArchetype(archetype: bigint): boolean {
