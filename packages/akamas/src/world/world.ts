import { type Class, Table } from '../components/mod.ts';
import { Entities, Entity } from '../entities/mod.ts';
import type { System } from '../systems/mod.ts';
import { devAssert } from '../utils/dev.ts';
import { getCompleteConfig, type WorldConfig } from './config.ts';
import type { Plugin } from './plugin.ts';
import type { Schedule, ScheduleType } from './schedule.ts';
...
export class World {
   ...
   */
  private pendingPlugins: Promise<any>[] = [];

  /**
   * A lookup for archetypes (`bigint`s) to tables.
   */
  private archetypeToTable: Map<bigint, Table> = new Map([
    [0n, this.tables[0]],
  ]);
   ...
   */
  private listeners: WorldEventListeners = {
    start: [],
    stop: [],
    createTable: [],
  };
  ...
  addEventListener<T extends keyof WorldEventListeners>(
    ...
    devAssert(
      ...
      `Unrecognized World event listener ("${String(type)}")`,
    );
